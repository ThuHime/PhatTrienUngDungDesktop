...
namespace Lab09_EntityFramework
{
    public partial class MainForm : Form
    {
        ...
        public MainForm(RestaurantContext context, AccountModel loggedInAccount)
        {
            InitializeComponent();
            _context = context;
        ...
        {
            if (_loggedInAccount == null || string.IsNullOrWhiteSpace(_loggedInAccount.AccountName))
            {
                menuAdminManagement.Visible = false;
                return;
            }

            // Include Role để đảm bảo EF load relation
            var roleAccount = _context.RoleAccounts
                .Include(ra => ra.Role)
                .FirstOrDefault(ra => ra.AccountName.Equals(_loggedInAccount.AccountName.Trim(), StringComparison.OrdinalIgnoreCase)
                                      && ra.Actived == true); // nếu bạn muốn chỉ dùng actived=true

            if (roleAccount == null)
            {
                // debug info: show all roleaccounts for this account
                var all = _context.RoleAccounts
                            .Where(ra => ra.AccountName == _loggedInAccount.AccountName)
                            .ToList();
                MessageBox.Show($"Không tìm thấy RoleAccount (actived) cho {_loggedInAccount.AccountName}. Tổng bản ghi RoleAccount cho account này: {all.Count}");
                menuAdminManagement.Visible = false;
                return;
            }

            if (roleAccount.Role == null)
            {
                MessageBox.Show($"RoleAccount tìm thấy nhưng Role NULL. RoleID trong RoleAccount = {roleAccount.RoleId}. Hãy kiểm tra bảng Role có ID này hay không.");
                menuAdminManagement.Visible = false;
                return;
            }

            var roleName = roleAccount.Role.RoleName ?? "";
            menuAdminManagement.Visible = roleName.Equals("Admin One", StringComparison.OrdinalIgnoreCase)
        ...
        private void LoadBillDetails(int tableId)
        {
            ...
            if (bill == null)
            {
                // thử tìm luôn bill bất kỳ (phòng khi bàn đó chỉ có bill đã thanh toán)
                bill = _context.Bills.FirstOrDefault(b => b.TableId == tableId);
            }
            ...
            var billDetails = _context.BillDetails
                .Include(bd => bd.Food)
                .Where(bd => bd.InvoiceId == bill.Id)
                .ToList();

            foreach (var detail in billDetails)
            {
                var item = new ListViewItem(detail.Food?.Name ?? "(Không có món)");
        ...
        private void listViewTables_Click(object sender, EventArgs e)
        {
            if (listViewTables.SelectedItems.Count > 0)
            {
                int tableId = (int)listViewTables.SelectedItems[0].Tag;
                LoadBillDetails(tableId);
            }
        }

        private void btnAddFoodToBill_Click_1(object sender, EventArgs e)
        {
            if (listViewTables.SelectedItems.Count == 0)
            {
                MessageBox.Show("Vui lòng chọn bàn.");
